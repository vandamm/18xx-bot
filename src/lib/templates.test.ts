import { DEFAULT_CONFIGURATION_MESSAGE, notificationMessage, processConfigurationMessage } from './templates';
...
test('configurationMessage requires webhook url', () => {
  expect(() => processConfigurationMessage(DEFAULT_CONFIGURATION_MESSAGE, 1, '')).toThrow(
    'Base URL is undefined'
  );
});

test('configurationMessage matches template', () => {
  expect(processConfigurationMessage(DEFAULT_CONFIGURATION_MESSAGE, 1, 'https://test.com')).toMatchInlineSnapshot(`
...
describe('processConfigurationMessage', () => {
  ...
  test('requires template', () => {
    expect(() => processConfigurationMessage('', 1, 'https://test.com')).toThrow(
      'Configuration message template is undefined'
    );
  });

  test('replaces single placeholder', () => {
    const template = 'Use this URL: {{WEBHOOK_URL}}';
    const result = processConfigurationMessage(template, 123, 'https://example.com');
    expect(result).toBe('Use this URL: https://example.com/send-notifications/123');
  });

  test('replaces multiple placeholders', () => {
    const template = 'URL: {{WEBHOOK_URL}} and again: {{WEBHOOK_URL}}';
    const result = processConfigurationMessage(template, 456, 'https://test.com');
    expect(result).toBe('URL: https://test.com/send-notifications/456 and again: https://test.com/send-notifications/456');
  });
  ...
  test('replaces USER_ID placeholder', () => {
    ...
    const result = processConfigurationMessage(template, 789, 'https://example.com');
    expect(result).toBe('Your ID: 789');
  });

  test('replaces all three placeholders together', () => {
    const template = 'Full URL: {{WEBHOOK_URL}}, Base: {{WEBHOOK_BASE_URL}}, ID: {{USER_ID}}';
    const result = processConfigurationMessage(template, 456, 'https://test.com');
    expect(result).toBe('Full URL: https://test.com/send-notifications/456, Base: https://test.com/send-notifications, ID: 456');
  });

  test('handles template with no placeholders', () => {
    const template = 'This is a message without placeholders';
    const result = processConfigurationMessage(template, 789, 'https://test.com');
...
🚀 You're all set! You'll receive notifications here when it's your turn.`;
    ...
    const result = processConfigurationMessage(template, 123456789, 'https://ping.vansach.me');
...
Both methods work the same way!`;
    ...
    const result = processConfigurationMessage(template, 555, 'https://api.example.com');
